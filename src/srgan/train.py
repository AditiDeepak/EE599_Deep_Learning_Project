...
def visualize(gen_inputs, real_inputs, fake_inputs):
    transform = torchvision.transforms.Compose([torchvision.transforms.Normalize(mean=[-2.118, -2.036, -1.804],
                                            ...
                                            torchivision.transforms.Scale(Config['img_size'])
                                            ])
...
def train():
    ...
    for epoch in range(Config['epochs']):
        ...
        for i, data in enumerate(dataloader):
            ...
            opt_generator.step()
            print(f'Epoch {epoch} Iter {i/len(data_loader)}: MSE Loss {generator_content_loss.data[0]}')
            writer.add_image('Pretrain SR Generator',visualize(low_res, high_res_real.cpu().data, high_res_fake.cpu().data), global_step = epoch)
        writer.add_scalar('generator_mse_loss', mean_generator_content_loss/len(data_loader), epoch)
    torch.save(generator.state_dict(), os.path.join(Config['checkpoint_path'],'generators'))
    ...
    for epoch in range(Config['epochs']):
        ...
        for i, data in enumerate(data_loader):
            ...
            high_res_fake = generator(torch.autograd.Variable(low_res))

            discriminator.zero_grad()
            discriminator_loss = adversarial_loss(discriminator(high_res_real), target_real) + adversarial_loss(high_res_fake, target_fake)
            ...
            real_features = torch.autograd.Variable(feat_extractor(high_res_real).data)
            fake_features = feat_extractor(high_res_fake)

            generator_content_loss = content_loss(high_res_fake, high_res_real) + 0.006 * content_loss(fake_features, real_features)
            mean_generator_content_loss += generator_content_loss.data[0]

            generator_adversarial_loss = adversarial_loss(discriminator(high_res_fake), ones_const)
