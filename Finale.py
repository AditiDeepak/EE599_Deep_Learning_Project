...
class Background_Removal_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.cat((self.transform(input_image.astype(np.float32)/255.0), self.transform(trimap_image.astype(np.float32)/255.0)),0), self.transform(target_image.astype(np.float32)/255.0)
